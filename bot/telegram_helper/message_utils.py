...
from bot.ext_utils.bot_utils import get_readable_message, setInterval
...
def sendMedia(text: str, bot, message: Message, reply_markup: InlineKeyboardMarkup):
    link = "http://inspirobot.me/api?generate=true"
    f = requests.get(link)
    imgurl=f.text
    try:
        return bot.sendPhoto(message.chat_id, reply_to_message_id=message.message_id, caption=text,
                             reply_markup=reply_markup, photo=imgurl,
                             parse_mode='HTMl')
    except RetryAfter as r:
        LOGGER.warning(str(r))
        sleep(r.retry_after * 1.5)
        return sendMedia(text, bot, message, reply_markup)
    except Exception as e:
        LOGGER.error(str(e))
        return
