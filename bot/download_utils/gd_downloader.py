...
from bot.status_utils.gd_download_status import GdDownloadStatus
from bot.telegram.message_utils import sendMessage, sendStatusMessage, sendMarkup
from bot.ext_utils.fs_utils import get_base_name


def add_gd_download(link, listener, is_gdtot):
    res, size, name, files = GoogleDriveHelper().link)
    if res != "":
